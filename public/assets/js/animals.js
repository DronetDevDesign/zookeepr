...
const getAnimals = (formData = {}) => {
  ...
  Object.entries(formData).forEach(([key, value]) => {
    queryUrl += `${key}=${value}&`; // in api HTTP address <<<
  });

  console.log(queryUrl);

  // getting the URL to fetch the animals
  fetch(queryUrl)
    .then(response => {
      if (!response.ok) {
        return alert('Error: ' + response.statusText);
      }
      return response.json(); // must use json() method to parse response into readable json format
    })
    .then(animalData => {
